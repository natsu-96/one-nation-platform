...
import { 
    MoreVertical, X, Link2, Plus, LogOut, ArrowLeft,
    Users, Clock, Trophy, BarChart3, Vote, Upload,
    Video, ShieldCheck, Award, Check, ChevronRight, Music
} from "lucide-react"; 
import UploadTalentDrawer from "./UploadTalentDrawer"; 
import defaultAvatar from "../assets/avatar.webp";
import "./DashboardDrawer.css";

// 🎯 Added onUploadSuccess to props destructured block
function DashboardDrawer({ isOpen, onClose, onLogout, onUploadSuccess }) {
    const [activeView, setActiveView] = useState("DASHBOARD");
    
    const userRole = localStorage.getItem("userRole") || "user";
    const currentEmail = localStorage.getItem("userEmail") || "kimdawap@gmail.com";

    // 🎯 RECOVER AUTOGENERATED ACCOUNT VALUES FROM CACHE
    const storedUsername = localStorage.getItem("username") || "Kim";
    const storedAvatar = localStorage.getItem("userAvatar");
    
    // Fallback chain: If admin, use shield icon. If custom avatar exists, use it. Otherwise, use static asset file.
    const displayAvatar = userRole === "admin" 
        ? "https://img.icons8.com/color/96/shield.png" 
        : (storedAvatar && storedAvatar !== "null" ? storedAvatar : defaultAvatar);
    ...
    const handleCopyLink = () => {
        const link = userRole === "admin" ? "https://naijacone.com/admin/portal" : `https://naijacone.com/profile/${storedUsername.toLowerCase()}`;
        navigator.clipboard.writeText(link);
        alert("Link copied to clipboard!");
    };
            ...
            <div className={`dashboard-side-drawer ${isOpen ? "open" : ""}`}>
                ...
                {activeView === "UPLOAD" && (
                    <UploadTalentDrawer 
                        onBackToDashboard={() => {
                            setActiveView("DASHBOARD");
                            // 🔥 TRIGGER SYNC PROCESS UPSTREAM UPON PANEL DRAW-BACK
                    ...
                    <>
                        ...
                        <div className="dashboard-scrollable-content">
                            <div className="profile-identity-section">
                                ...
                                <div className="identity-details">
                                    <h3 className="profile-display-name">
                                        {userRole === "admin" ? "Admin" : storedUsername}
                                    </h3>
                                    <p className="profile-handle-email">{currentEmail}</p>
                                    {userRole === "admin" && <span className="super-admin-badge">⚡ Super Admin</span>}
                                    <button className="copy-link-btn" onClick={handleCopyLink}>
                        ...
                        <div className="drawer-top-actions" style={{justifyContent: 'flex-start', borderTop: '1px solid var(--border-color)', padding: '12px 24px'}}>
                            <button className="bottom-logout-trigger-btn" onClick={handleLogoutWrapper}>
                                <LogOut size={18} />
                                <span>Logout</span>
                            </button>
