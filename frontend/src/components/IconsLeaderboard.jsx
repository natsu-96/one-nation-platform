...
function IconsLeaderboard({ 
  ...
  const totalVotes = icons.reduce((sum, icon) => sum + icon.votes, 0);
        ...
        <section className="leaderboard-section">
          <div className="leaderboard-list">
            {topFive.map((player, index) => {
              const iconsName = icons.name || "Anon Artist";
              return (
              <div key={player.id || index} className="leaderboard-row">
                <span className="rank-number">{index + 1}</span>
                <span className="player-name">{iconsName}</span>
...
export default IconsLeaderboard;